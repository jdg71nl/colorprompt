...
alias ng='f(){ echo "# alias ng => lsof -i -n -P +c0 | grep \$@  " ; lsof -i -n -P +c0 | egrep "PID|$1" ; unset -f f; }; f'
...
fifmp ()  { echo "# bash-function, see 'type fifmp':"; gfind . -path '*/node_modules/*' -prune -o -xtype f -iname '*'$2'*' -print0 | gxargs -0i grep -sinH "$1" "{}"; }
# on busybox/alpine
fifb ()  { echo "# bash-function, see 'type fifb':"; /usr/bin/find . -type f -iname '*'$2'*' -print0 | xargs -0i grep -sinH "$1" "{}"; }
# alt:
# > brew install ffind
# > brew install rargs
